...
public class VFL implements ModelInterface {
    ...
    public void prepFinal() {
        if (vflLoopOrder != null) {
            Log.warning(getClass(), ("\" vflLoopOrder \" is set, the order of vfl is " + vflLoopOrder.name()));
            parent.pulseTX180.prepTxAmpMultiFA(parent.getListInt(CommonUP.TX_ROUTE), parent.getListDouble(UP.VFL_FLIP_ANGLE_TRAIN), vflLoopOrder);
        } else {
            Log.warning(getClass(), "\" vflLoopOrder \" is null, default order \" LoopB \" is used");
            parent.pulseTX180.prepTxAmpMultiFA(parent.getListInt(CommonUP.TX_ROUTE), parent.getListDouble(UP.VFL_FLIP_ANGLE_TRAIN), Order.LoopB);
        }
    }
    ...
    public ArrayList<Double> getVFL() {
        if (isVFLEnabled) {
            if (isLongVFL) {
                ...
                ArrayList<Double> pseudoVFA = new ArrayList<>(Collections.nCopies(parent.getInt(CommonUP.ECHO_TRAIN_LENGTH), parent.getDouble(CommonUP.FLIP_ANGLE_REFOC)));
                pseudoVFA.set(0, parent.getDouble(CommonUP.FLIP_ANGLE) + parent.getDouble(CommonUP.FLIP_ANGLE_REFOC) / 2);
                return pseudoVFA;
            }
        } else {
            return new ArrayList<>(Collections.nCopies(parent.getInt(CommonUP.ECHO_TRAIN_LENGTH), parent.getDouble(CommonUP.FLIP_ANGLE_REFOC)));
        }
    }
    ...
    public double getEffTE() {
        if (isLongVFL)
            return getEffTE(parent.getDouble(CommonUP.ECHO_TIME), parent.getDouble(CommonUP.ECHO_SPACING), parent.getInt(CommonUP.ECHO_TRAIN_LENGTH));
        else
            return parent.getDouble(CommonUP.ECHO_TIME);
    }

    public double getInvEffTE() {
        return getInvEffTE(parent.getDouble(CommonUP.ECHO_TIME), parent.getDouble(CommonUP.ECHO_SPACING), parent.getInt(CommonUP.ECHO_TRAIN_LENGTH));
    }

    public void setVFLOrder(Order order) {
        // we provide an API for user input order
        // by default, we take Order.LoopB
        vflLoopOrder = order;
    }
    ...
    private void iniETLandVFL() throws Exception {
        ...
        if (fa_min <= 0.0)
            fa_min = Math.max(parent.getDouble(CommonUP.FLIP_ANGLE_REFOC) / 6.0 + 5.0, 20.0);
        if (fa_mid <= 0.0)
            fa_mid = parent.getDouble(CommonUP.FLIP_ANGLE_REFOC) / 2.0 + 10.0;

        if (isVFLEnabled && parent.echoTrainLength >= 20 && parent.getDouble(CommonUP.FLIP_ANGLE_REFOC) < 180.0) {
            ...
            while (Math.floorMod(echoTrainLengthEncoded, 2) != 0) { //we need a even number of echoTrainLengthEncoded //Maybe NOT...
                getVFL(fa_min, fa_mid, parent.getDouble(CommonUP.FLIP_ANGLE_REFOC), parent.echoTrainLength); // our method is independent of ESP
        ...
        parent.getParam(CommonUP.ECHO_TRAIN_LENGTH).setValue(parent.echoTrainLength);
