...
public class FlyBack implements ModelInterface {
    ...
    protected void initPulseandGrad() throws Exception {
        gradReadoutFlyback = Gradient.createGradient(parent.getSequence(), SP.Grad_amp_flyback, SP.Time_flyback,
                CommonSP.Grad_shape_rise_up, CommonSP.Grad_shape_rise_down, SP.Time_grad_ramp_flyback, parent.nucleus);
    }

    protected void prepGrad() {
        if (isFlyBackEnabled) {
            double grad_shape_rise_time = parent.clcGradEqRiseTime(CommonSP.Grad_shape_rise_up, CommonSP.Grad_shape_rise_down, parent.getDouble(CommonUP.GRADIENT_RISE_TIME));
    ...
    protected void setSeqParamTime() {
        ...
        time_flyback_ramp = isFlyBackEnabled ? parent.getDouble(CommonUP.GRADIENT_RISE_TIME) : parent.minInstructionDelay;
